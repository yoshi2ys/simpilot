...
final class HelpHandler {
    ...
    private func buildCommands() -> [[String: Any]] {
        return [
            ...
            cmd("appearance", "GET/POST", "Get or set appearance (light/dark mode)",
                [arg("mode", required: false, "light, dark, or unspecified (omit to get current)")],
                "simpilot appearance dark"),
            cmd("location", "POST", "Simulate GPS location (iOS 17+)",
                [arg("latitude", required: true), arg("longitude", required: true)],
                "simpilot location 35.6812 139.7671"),
