...
enum HelpCommands {
    ...
    static let all: [CommandInfo] = [
        ...
        .init(name: "stop", category: .agent,
              synopsis: "stop (--port <p> | --udid <u> | --all)",
              description: "Stop one or all running agents (requires a target)",
        ...
        .init(name: "assert", category: .interaction,
              synopsis: "assert <exists|not-exists|enabled|hittable|stable|value|label> <query> [expected] [--timeout <s>] [--snapshot-on-fail]",
        ...
        .init(name: "screenshot", category: .observation,
              synopsis: "screenshot [--file <path>] [--scale <N|native>]",
              description: "Capture a screenshot (default --scale 1 = 1x/point-size for AI; use 'native' for device full resolution)",
              example: "simpilot screenshot --file /tmp/s.png --scale native"),
