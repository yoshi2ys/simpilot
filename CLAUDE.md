...
- **Runner teardown by bundle ID** (StopCommand / SimctlHelper.terminateRunner): the simulator-side `AgentUITests-Runner` owns the listening socket and is parented by `launchd_sim`, so SIGTERMing `xcodebuild` alone leaves the port bound. `stop` also runs `simctl terminate <udid> dev.yoshi.simpilot.AgentUITests.xctrunner`; `stop --all` sweeps every booted simulator the same way. Scoping by bundle ID is exact where a `pgrep -f` pattern would have to guess.
