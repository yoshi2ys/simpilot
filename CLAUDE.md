...
- **`@eN` aliases** (`ElementSnapshot` / `AliasResolver`, SU3): `elements --format outline` records what each line was, so `tap '@e11'` names a line instead of re-describing the element. This is the only handle for elements a query cannot name at all — on the iOS Settings root, 12 of 45 actionable elements carry neither label nor identifier and 24 of 45 share their `(type, label, identifier)` with another element. Aliases number the **unfiltered** actionable list, so `--type`/`--contains` show gaps (`@e11`, `@e14`) rather than renumbering from `@e1`: renumbering per filter would make `@e1` mean something different depending on flags the next command does not repeat. An alias is **re-validated, never trusted** — resolution re-derives the list from the live tree and checks position N still holds the recorded identity, then uses that element's *current* geometry. Frames are excluded from the identity on purpose: scrolling moves every row without changing which row it is. `no_snapshot` (no outline taken yet) and `stale_alias` (app switched, list shorter, or identity changed) are distinct codes because the repair differs from `element_not_found` — the fix is always to list the screen again, never to hunt for a label.
